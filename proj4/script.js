// (() => {
//   // ---------- UI bits (kept minimal) ----------
//   const bar = document.querySelector('.progress .bar');
//   const scrollEl = document.scrollingElement || document.documentElement;
//   function updateProgress() {
//     const h = scrollEl.scrollHeight - scrollEl.clientHeight;
//     if (bar) bar.style.width = (h > 0 ? (scrollEl.scrollTop / h) * 100 : 0).toFixed(2) + '%';
//   }
...
//   // ---------- Starfield core ----------
//   const canvas = document.getElementById('sky');           // already in your HTML
...
//     const CFG = {
//       dprMax: 1,
//       parallax: { far: 0.015, mid: 0.16, near: 0.64},
//       twinkle:  { mid: 0.20, near: 0.30 },
//       jwstRatio: 0.10,
//       tile: { dust: 640, mid: 1024, near: 48 },
//       perMP: { mid: 180, near: 90},
//       dustPerTile: 2400,
//       driftX: 0.006
//     };
...
//         g.addColorStop(1,`rgba(255,255,255,0)`);
//         d.fillStyle=g; d.beginPath(); d.arc(x,y,r*3,0,TWO); d.fill();
//       }
...
//         const phi = Math.random()*TWO, rot = Math.random()*TWO;

//         const a0 = clamp(base + amp*Math.cos(phi + 0*TWO/3), 0, 1);
//         const a1 = clamp(base + amp*Math.cos(phi + 1*TWO/3), 0, 1);
//         const a2 = clamp(base + amp*Math.cos(phi + 2*TWO/3), 0, 1);
...
//       const ang = timeSec * TWO * twinkleSpeed;
//       let w0 = Math.max(0, Math.sin(ang) + 1);
//       let w1 = Math.max(0, Math.sin(ang + TWO/3) + 1);
//       let w2 = Math.max(0, Math.sin(ang + 2*TWO/3) + 1);
...
//         this.x=x; this.y=y; this.a=a; this.vx=Math.cos(a)*speed; this.vy=Math.sin(a)*speed;
//         this.len=len; this.h=hue; this.life=0; this.max=lifeMs; this.thick=Math.max(1*DPR,1.6*DPR); this.alive=true;
//       }
...
//         g.addColorStop(0,`hsla(${this.h},60%,95%,${0.85*alpha})`); g.addColorStop(1,`hsla(${this.h},80%,60%,0)`);
...
//       const time = 2.3; // seconds
//       const speed = dist / time;
//       const len = Math.min(0.35 * dist, 320*DPR);
//       const lifeMs = (time*1000) + 300;
...
//   const renderer = createStarfieldRenderer(ctx, { reduceMotion });

//   function resize() {
//     renderer.resize(innerWidth, innerHeight, devicePixelRatio);
//     // match CSS size to avoid scaling blur
...
//     last = now;
//     renderer.step(dt, scrollY); // snap directly to scroll; no easing
...
/* Starfield + UI (single-file, no worker)
   - Mobile profile: DPR=1, fewer stars, no grain, ~30fps
   - Scroll snap (no easing drift)
...
(() => {
  ...
  function frame(now){
    if (paused) { return; }
    const dt = Math.min((now - last)/1000, MOBILE_LOW ? 0.05 : 0.04);
